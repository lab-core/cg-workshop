...
def main() -> int:
    ...
    mode.add_argument("--Ks", type=int, nargs="+",
                      default=None,
                      metavar="K",
    ...
    ap.add_argument("--alpha", type=float, default=0.0,
                    help="Wentges alpha, used when --Ks is active (default: 0)")
    ...
    # Build (label, K_MAX, alpha) triples.
    if args.Ks is not None:
        configs = [(f"K = {k}", k, args.alpha) for k in args.Ks]
        mode_desc = f"alpha={args.alpha}"
    else:
        alphas = args.alphas if args.alphas is not None else [0.0, 0.1, 0.3, 0.5]
        configs = [(f"α = {a}", args.K, a) for a in alphas]
        mode_desc = f"K_MAX={args.K}"
    ...
    for label, k_max, alpha in configs:
        ...
        vrp = VRP(inst, K_MAX=k_max, alpha=alpha)
