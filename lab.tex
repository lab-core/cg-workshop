...
\begin{frame}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    \node[anchor=south east, xshift=-60mm, yshift=10mm]
          ...
          {\begin{tikzpicture}
           ...
           \end{tikzpicture}};
    \node[anchor=south east, xshift=-62mm, yshift=6mm,
...
\begin{frame}{What this deck contains}
  \begin{itemize}\setlength{\itemsep}{4pt}
    ...
    \item File map of the \texttt{python/} folder.
...
\begin{frame}[fragile]{Create the virtual environment}
  ...
  \texttt{\scriptsize cd python/ \&\& python3 -m venv .venv \&\&
  source .venv/bin/activate}\\[2pt]
  \textbf{Windows PowerShell:}\hspace{1em}
  \texttt{\scriptsize cd python/ ; python -m venv .venv ;
...
\end{lstlisting}
  Most checks default to \texttt{toy.txt}; \texttt{exB\_pricing} defaults to
  \texttt{R101\_25.txt}. Pass \texttt{{-}{-}instance <name>} to override.
\end{frame}
...
\begin{frame}[fragile]{The \texttt{python/} folder}
\begin{lstlisting}[style=sh,basicstyle=\ttfamily\tiny]
python/
+-- README.md, requirements.txt, theory.pdf, lab.pdf
+-- instances/                 toy.txt + Solomon files (+ duals/<inst>/iter_0.txt)
...
\begin{frame}{The TODO holes -- complete checklist}
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    \textbf{EX-C.4} & \texttt{vrp.py} & per-iteration log line (LP, LB, UB, gap) \\
...
\begin{lstlisting}[style=sh]
[ex.A] instance: toy.txt (6 customers, K=6 vehicles)
[ex.A] building master with 6 initial columns
[ex.A] LP optimum (with slacks): 228.30
[ex.A] vehicle dual sigma     : -0.00
[ex.A] customer duals:
[ex.A]    pi_1=42.43  pi_2=41.23  pi_3=22.36
[ex.A]    pi_4=41.23  pi_5=50.00  pi_6=31.05
[ex.A] OK -- master is feasible and returns finite duals.
\end{lstlisting}
...
\begin{lstlisting}[style=sh]
[ex.B] loading saved duals from instances/duals/R101_25/iter_0.txt
[ex.B] best route        : [0, 14, 15, 3, 24, 25, 26]
[ex.B] reduced cost      : -138.98
[ex.B] # neg-cost routes : 58
[ex.B] OK
\end{lstlisting}
  \begin{warnbox}[Common bug]
    Forgetting that the depot's dual is $0$ leaves a phantom $-\infty$ on
    every depot-to-depot arc. Initialise \texttt{pi[0] = $\sigma$} explicitly.
  \end{warnbox}
\end{frame}
...
\begin{frame}{Exercise C -- goal}
  \begin{exbox}[Exercise C -- multi-column, smoothing, LB]
    ...
    \begin{itemize}\small
      \item \textbf{C.2}: cap pricer output at \texttt{nb\_cols} columns; only
    ...
    \end{itemize}
    Try \texttt{{-}{-}nb-cols-list 1\,5\,20\,50} and \texttt{-a 0\,0.3\,0.5}
...
\begin{frame}[fragile]{Exercise C -- expected output (R101\_25, K=25)}
\begin{lstlisting}[style=sh,basicstyle=\ttfamily\scriptsize]
(.venv) $ python -m vrp.tests.exC_cg
[cg] iter cols  LP        LB         gap%
[cg]    0   75  1246.16  -2228.39   155.9%
[cg]    1   90   703.94   -447.45   257.3%
[cg]    2   95   674.01      8.45   7877%
[cg]    3  100   665.92    283.68   134.7%
[cg]    4  104   640.45    444.00    44.2%
[cg]    5  110   640.45    375.12    70.7%
[cg]    6  114   637.33    547.37    16.4%
[cg]    7  114   636.28    636.28     0.0%
[cg] CG done in 8 iters, LP=636.28, LB=636.28, 89 cols added
\end{lstlisting}
  Try \texttt{{-}{-}nb-cols} $\in\!\{1,5,20,100\}$ and observe the iteration count drop.
  Wentges $\alpha\!=\!0.5$ often reduces iterations. Without \texttt{{-}{-}nb-vehicles 25}
  the first 3 iterations show slack-inflated LP/LB values.
\end{frame}
...
\begin{lstlisting}[style=sh,basicstyle=\ttfamily\scriptsize]
[ex.D] root LP=636.3
[ex.D] RMH incumbent (UB)=636.3, time=0.1s
[ex.D] RMH solution:
[ex.D]   cost=81.11  [0, 11, 19, 10, 26]
[ex.D]   cost=59.67  [0, 7, 8, 26]
[ex.D]   cost=46.47  [0, 2, 21, 26]
...  (9 routes, total 636.28)
[diving] dive start: LP=636.28
[diving] dive end (integer): 0 fixes, UB=636.28
[ex.D] summary: RMH UB=636.3   dive UB=636.3   LP=636.3
\end{lstlisting}
  \begin{tipbox}[LP already integer on R101\_25]
    The CG LP is integer for this instance -- RMH solves the exact MIP and
    the dive terminates with 0 fixes.  On harder instances (e.g.\
    \texttt{C101\_25.txt} or \texttt{RC101.txt}) you will see actual fixes
    and a gap between LP and RMH. A good way to make the instance harder is to decrease the number of vehicles with \texttt{{-}{-}nb-vehicles}.
  \end{tipbox}
\end{frame}
...
\begin{lstlisting}[style=sh,basicstyle=\ttfamily\scriptsize]
(.venv) $ python -m vrp.tests.exE_bnp
[bnp] B&P start: root LB=1636.39, 1370 columns in pool, depth_first=True ...
[bnp] B&P done in 12 nodes, optimum=1642.88 (20 routes), global LB=1642.88 ...
[ex.E] done in 5.8s, optimum=1642.88
[ex.E] solution (20 routes):
[ex.E]   cost=86.77  [0, 92, 42, 15, 87, 57, 97, 101]
[ex.E]   cost=50.00  [0, 31, 88, 7, 101]  ...


\end{lstlisting}
  \begin{tipbox}[1 node = proven optimal without branching]
    R101\_25 has an integer root LP -- the B\&P terminates immediately.
    On larger or more constrained instances (\texttt{RC101.txt}, or
    \texttt{C101\_25.txt}) the tree has many nodes and branching is
    essential. Use \texttt{compare\_bnp} to observe the search progress.
  \end{tipbox}
\end{frame}
...
\begin{frame}[fragile]{Five errors that always show up}
  ...
  \begin{tipbox}[Get unstuck fast]
    The saved \texttt{iter\_0.txt} in \texttt{instances/duals/R101\_25/} is a
    one-iteration regression check. Diff your duals against it.
  \end{tipbox}
\end{frame}
