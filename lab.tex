...
\end{lstlisting}
  ...
  Without \texttt{{-}{-}nb-vehicles 25} the first iterations show slack-inflated values.
\end{frame}
...
\end{lstlisting}
  \begin{tipbox}[LP already integer on R101\_25]
    ...
    and a gap between LP and RMH. Decrease the number of vehicles with
    \texttt{{-}{-}nb-vehicles} to make the instance harder.
  \end{tipbox}
\end{frame}
...
[ex.E] done in 5.9s, optimum=1642.88
[ex.E] solution (20 routes):
[ex.E]   cost=86.77  [0, 92, 42, 15, 87, 57, 97, 101]
[ex.E]   cost=50.00  [0, 31, 88, 7, 101]
[ex.E]   cost=107.35  [0, 63, 64, 49, 48, 101] ...
\end{lstlisting}
  \begin{tipbox}[1 node = proven optimal without branching]
    R101\_25 has an integer root LP (you can try) -- the B\&P terminates immediately.
    On larger or more constrained instances (\texttt{RC101.txt} here) the tree has many nodes and branching is essential. Use \texttt{compare\_bnp} to observe the search progress.
  \end{tipbox}
\end{frame}
