...
\subtitle{A 4-hour practical workshop\\ \small{Modeling -- Pricing -- Integrality -- Optimality}}
...
\begin{frame}{4-hour outline}
  ...
  \vspace{0.5em}
  \begin{itemize}\small
    \item Hands-on coding alternates with the slides.
  \end{itemize}
\end{frame}
...
\begin{frame}{The basic loop, in one slide}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{tikzpicture}[every node/.style={font=\scriptsize}]
      ...
      \draw[->,thick,accent2] (rmp.east) -- node[above,scale=0.95]{1. duals $\boldsymbol\pi^*$} (sub.west);
      \draw[->,thick,accent]  (sub.south west)
           .. controls +(-0.8,-1.2) and +(0.8,-1.2) ..
           node[below,align=center,scale=0.95]{2. add $p^*$ with\\$\redcost_{p^*}\!<\!0$}
           (rmp.south east);
      \draw[->,thick] (sub.south) -- node[right,scale=0.9]{3. all $\redcost\!\ge\!0$} (stop.north);
    \end{tikzpicture}
...
\begin{frame}{What is the pricer for cutting stock?}
...
\begin{frame}{What is the pricer for VRPTW?}
...
\begin{frame}[fragile]{First example end-to-end -- toy cutting stock}
  ...
  \begin{tikzpicture}[xscale=0.30,yscale=0.45,every node/.style={font=\tiny}]
    \begin{scope}[xshift=0cm]
      \node[above,scale=1.2] at (2.5,0.5){$p_1\!=\!(3,0,0)$};
...
\begin{frame}{Initial columns -- bootstrap}
  ...
  \begin{warnbox}[Why not start empty?]
    \pause With an infeasible $\rmp$ the dual is unbounded: the pricer gets bogus
...
\begin{frame}{Quick view -- Dantzig--Wolfe decomposition}
  ...
  $x=\sum_p \lambda_p\, v_p + \sum_r \mu_r\, r_r$, $\lambda\geq 0, \sum_p \lambda_p = 1$, $\mu\ge 0$
...
\begin{frame}{Exercise A -- Build the master in HiGHS}
...
\begin{frame}{A worked RCSPP example}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{tabular}{@{}lcc@{}}
      ...
      Path & $\sum c$ & $\sum\rho$ \\
      \midrule
      $o\!\to\!a\!\to\!c\!\to\!d$ & \only<2->{$5$} & \only<2->{$6$ \cmark} \\
      $o\!\to\!b\!\to\!c\!\to\!d$ & \only<2->{$3$} & \only<2->{$11$ \xmark} \\
      $o\!\to\!a\!\to\!d$         & \only<2->{$5$} & \only<2->{$8$ \xmark} \\
      \bottomrule
    \end{tabular}

    \medskip
    Optimum: \only<2->{$o\!\to\!a\!\to\!c\!\to\!d$, cost $5$.}
  \end{columns}
  \medskip
  \alert{Shortest path and feasible path are not the same.} \only<2->{The cheaper
...
\begin{frame}{Elementary or not? coefficient $a_{ip}$ in the master}
  ...
  \begin{itemize}\small
    ...
    \item \textbf{Non-elementary} (RCSPP): pricer may produce routes
  ...
  \end{tcolorbox}
  \begin{itemize}\small
    \item Elementary: tighter LP, harder pricer (ERCSPP).
...
\begin{frame}{ERCSPP vs RCSPP}
  \begin{tabular}{@{}p{0.2\textwidth}p{0.75\textwidth}@{}}
    \toprule
    \textbf{ERCSPP} & visit each node \alert{at most once}; pricing for
                      set-partitioning master with $=1$ constraints. \\
    \textbf{RCSPP}  & nodes may be revisited; pricing for
                      set-covering ($\ge 1$). \\
    \bottomrule
  \end{tabular}
  \medskip
  \begin{itemize}\small
    \item ERCSPP is \alert{strongly} NP-hard.
    \item RCSPP with non-negative reduced costs and \alert{integer}
          resources is solvable in pseudo-polynomial time.
    \item Most modern VRPTW solvers price ng-routes: between RCSPP and ERCSPP,
          they tighten dominance compared to RCSPP but stay tractable.
  \end{itemize}

  \begin{keybox}[Practical advice]
    Avoid solving the full \alert{ERCSPP} in general -- it explodes
    combinatorially. Start with \alert{RCSPP} pricing; if the master ends up
    with too many revisiting routes, switch to ng-routes. Reserve exact
    ERCSPP for small instances or for the last few branch-and-price nodes.
  \end{keybox}
\end{frame}
...
\begin{frame}{Heuristic vs exact pricing}
  ...
  \begin{itemize}\small
    ...
    \item Hierarchy: greedy $\to$ heuristic $to$ exact RCSPP / ERCSPP.
    \item For LP optimality, the \alert{last} pricing call must be exact.
    \item For the Lagrangian bound (Part 4), we also need a true lower bound
...
\begin{frame}{Returning multiple columns per call}
  ...
  \begin{tcolorbox}[colback=accent!5,colframe=accent!50,sharp corners,boxrule=0.4pt]
   In the lab, \texttt{VRP(NB\_COLS=N)} caps the number of columns added per
   CG iteration. The \texttt{compare\_cg} runner sweeps $N$ for you.
  \end{tcolorbox}
\end{frame}
...
\end{lstlisting}
  \begin{itemize}\small
    ...
    \item \alert{Build the graph once}; call
          \texttt{update\_reduced\_costs} every CG iteration.
  \end{itemize}
\end{frame}

\begin{frame}{Exercise B -- Wire the pricing subproblem}
...
\begin{frame}{Where the time goes}
  ...
  \end{center}
  \begin{itemize}\small
    \item Rule of thumb: pricing is 70-95\% of total time, should never be less than $40\%$.
...
\begin{frame}{Trick 2 -- Hierarchy of pricers}
  ...
  \begin{tikzpicture}[every node/.style={font=\small}]
    ...
    \draw[->,thick,accent] (g.east)  -- ++(0.5,0) -| node[right,scale=0.8,pos=0.25]{} (sp.north);
    \draw[->,thick,accent] (h.east)  -- ++(0.5,0) -| node[right,scale=0.8]{cols} (sp.north);
  \end{tikzpicture}
  \end{center}
  \vspace{-0.3em}
  \begin{itemize}\small
...
\begin{frame}{Path family is a separate choice}
  ...
  \begin{tabular}{@{}p{0.30\textwidth}p{0.62\textwidth}@{}}
    ...
    \midrule
    \textbf{RCSPP} & nodes can be revisited; coefficient $a_{ip}$ in master
    ...
    \textbf{ERCSPP} & every customer visited at most once. Strongest LP,
...
\begin{frame}{Trick 5 -- Fast LP re-optimisation}
  ...
  \begin{warnbox}[Beware]
    Building the LP from scratch each iteration is terrible for production. In the exercice, you will keep the LP alive across iterations.
  \end{warnbox}
\end{frame}
...
\begin{frame}{How do we bound the LP early?}
...
\begin{frame}{Trick 8 -- Symmetry and elementarity}
  \begin{itemize}\small
    ...
    \item Watch out for accidentally symmetric objectives: if your route cost
          ...
          everywhere.
    \item Elementarity vs RCSPP: revisits are LP-feasible only when the dual
...
\begin{frame}{The LP is solved -- now what?}
  \begin{itemize}\small
    ...
    \begin{itemize}\small
      ...
      \item \alert{Exact}: branch-and-price (Part 6): Branch-and-bound on the master IP, where every node's LP relaxation is solved by column generation.
    \end{itemize}
...
\begin{frame}{Exercise D -- Heuristic IP on the column pool}
...
\begin{frame}{How would you branch?}
  ...
  \begin{keybox}[Questions]
    \begin{itemize}
      \item Why does \alert{$x_p\!\le\!0$ vs $x_p\!\ge\!1$} (branching on the
            master variable) destroy the branch-and-bound tree and the pricer?
...
\begin{frame}{The naive branch (don't do this)}
  ...
  \begin{itemize}\small
    ...
    \item Down-branch: \alert{forbid one specific column} $p$. The pricer
          ...
          plain RCSPP and \alert{breaks dominance}.
...
\begin{frame}{Ryan-Foster branching (set-partitioning case)}
  ...
  \begin{itemize}\small
    \item \textbf{Together}: keep only routes that visit \alert{both or neither}.
          Heuristic: Implement by \emph{merging} $i$ and $j$ into a single node with combined demand and the time window
  ...
  \begin{tikzpicture}[every node/.style={font=\small}]
    ...
    \begin{scope}[xshift=-3.4cm,yshift=-0.4cm]
      ...
      \node[below=2mm of ij1,scale=0.8,align=center]
       {"heuristic" merge $i,j$ into one node;\\$q\!=\!q_i\!+\!q_j$, window "$[a_i,b_i]\!\cup\![a_j,b_j]$"};
...
\begin{frame}{Branching on arcs / flow}
  ...
  \begin{itemize}\small
    ...
    \item Often produces more balanced trees than Ryan-Foster, but deeper.
  \end{itemize}
\end{frame}
...
\item \Indent $N \leftarrow$ pick-next(open) \quad\textcolor{accent2}{\# best-first (smallest LB) OR depth-first dive}
...
\begin{frame}{Putting all of B\&P on one slide}
  ...
  \end{tikzpicture}
  \begin{itemize}\small
    \item Solid blue nodes = pruned by bound, integrality (or infeasibility).
...
\begin{frame}{Exercise E -- B\&P with Lagrangian pruning}
...
\begin{frame}{CG convergence -- Wentges smoothing ($\alpha$)}
  \centering
  \includegraphics[width=\textwidth]{plot/cg_alphas.png}
\end{frame}

\begin{frame}{CG convergence -- pricing batch size (\texttt{nb\_cols})}
  \centering
  \includegraphics[width=\textwidth]{plot/cg_cols.png}
\end{frame}

\begin{frame}{B\&P progress -- depth-first vs best-first}
  \centering
  \includegraphics[width=.9\textwidth]{plot/bnp_boths.png}
\end{frame}

\begin{frame}{B\&P progress -- pricing batch size (\texttt{nb\_cols})}
  \centering
  \includegraphics[width=.9\textwidth]{plot/bnp_cols.png}
\end{frame}

\begin{frame}{B\&P progress -- optimality-gap stop threshold}
  \centering
  \includegraphics[width=.9\textwidth]{plot/bnp_gaps.png}
\end{frame}
...
\begin{frame}{If you remember four things}
  \begin{enumerate}\setlength{\itemsep}{6pt}
    ...
    \item \textbf{Branch on structure, not on master variables}: Ryan-Foster,
          ...
          intact.
    \item \textbf{Use many heuristics} to implement an efficient CG and B\&P.
