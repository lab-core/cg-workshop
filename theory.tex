...
% Custom frame numbering "k / N" using the real total
...
\begin{frame}{Returning multiple columns per call}
  ...
  \begin{tcolorbox}[colback=accent!5,colframe=accent!50,sharp corners,boxrule=0.4pt]
   In the lab, the function \texttt{solve\_subproblem(\dots, max\_nb\_solutions)}
   already returns a list of solutions. You'll experiment with the cap.
  \end{tcolorbox}
\end{frame}
...
from rcspp.resource import (
    RealAdditionExtensionFunction,        # cost or load
    RealValueDominanceFunction,           # <= dominance
    RealValueCostFunction,
    RealTrivialFeasibilityFunction,       # no bound
    MinMaxFeasibilityFunction,            # [lo, hi]
    TimeWindowExtensionFunction,          # max(T+travel, a_v)
    TimeWindowFeasibilityFunction,        # T <= b_v
)
g = ResourceGraph()
g.add_real_resource(ext, feas, cost, dom)   # one call per resource
g.add_node(node_id, is_source, is_sink)
g.add_arc(([(redcost,), (time,), (load,)],), o, d, arc_id, redcost)
solutions = g.solve()                        # returns Solution[]
\end{lstlisting}
  \begin{itemize}\small
    \item Each arc carries a \alert{tuple of resource increments}, in the same
          order as registered resources.
    \item \texttt{solve()} runs label-setting and returns
          \alert{all sink labels with negative cost}, sorted.
  \end{itemize}
\end{frame}
...
\item incumbent $\leftarrow +\infty$; tree $\leftarrow\{$root$\}$
\item \textbf{while} tree non-empty
\item \Indent pick node $N$; pool$_N$ $\leftarrow$ filter(parent pool, branch)
\item \Indent run CG on $N$, with Lagrangian bound LB at every iteration
\item \Indent \textbf{if} LB $\ge$ incumbent: \textbf{prune}
\item \Indent \textbf{if} LP integer: incumbent $\leftarrow\min(\text{incumbent}, z^{\rmp})$; \textbf{prune}
\item \Indent run heuristic (RMH/dive) on pool$_N$; update incumbent
\item \Indent pick branching variable; create children
\item \textbf{end while}
\end{algo}
  \begin{itemize}\small
    \item Node selection: best-first by LB, then DFS for memory.
    \item Pool filter: easy with arc branching (drop columns using forbidden
          arc), trickier with Ryan-Foster (drop columns visiting both / drop
          columns visiting one xor the other).
  \end{itemize}
\end{frame}
...
\begin{frame}{\textbf{[lab]} Exercise E -- B\&P with Lagrangian pruning}
  \begin{exbox}[Exercise E -- arc-branching B\&P]
    ...
    bound is reused from Exercise C / Trick~7.
...
\begin{frame}{Hand-off to the lab}
  \begin{itemize}\small
    \item All five exercises (A--E) are in the separate \alert{lab kit},
          with a per-exercise README and unit tests.
    \item Open the \texttt{lab/} folder shipped with this deck.
    \item The lab deck (\texttt{lab.pdf}) walks you through installation,
          file map, and the expected outputs.
  \end{itemize}
\end{frame}
...
\begin{frame}{Hand-off to the lab kit}
  ...
  \begin{itemize}\small
    ...
    \item the directory layout of the \texttt{participants/} folder;
